...
def is_ckpt_valid(ckpt_dir):
    ...
    if len(list(pathlib.Path(ckpt_dir).glob("*.safetensors"))) == 0:
        return False
    if len(list(pathlib.Path(ckpt_dir).glob("rng_state_*.pth"))) == 0:
        return False
...
def check_ckpt_exists(output_dir):
    if is_main_process():
        while list(pathlib.Path(output_dir).glob("checkpoint-*")):
            ckpt_paths = list(pathlib.Path(output_dir).glob("checkpoint-*"))
            ckpt_iters = [int(path.name.split('checkpoint-')[-1]) for path in ckpt_paths]
            max_ckpt_iter = max(ckpt_iters)
            max_ckpt_path = ckpt_paths[ckpt_iters.index(max_ckpt_iter)]
            if is_ckpt_valid(max_ckpt_path):
                break
            shutil.rmtree(max_ckpt_path)
