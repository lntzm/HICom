...
EVAL_DATA_DIR=playground/data/eval_video/MLVU
OUTPUT_DIR=work_dirs/eval_output/MLVU
CKPT=${1}
...
if [ ! -f "$output_file" ]; then
    for IDX in $(seq 0 $((CHUNKS-1))); do
        ...
        TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=${gpu_devices} python3 hicom/eval/video/inference_video_mcqa_mlvu.py \
            --model-path ${CKPT} \
            --video-folder ${EVAL_DATA_DIR}/MLVU/video \
            --question-file ${EVAL_DATA_DIR}/MLVU/json \
...
python3 hicom/eval/video/eval_video_mcqa_mlvu.py \
