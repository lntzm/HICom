...
EVAL_DATA_DIR=playground/data/eval_video/Activitynet_Zero_Shot_QA
OUTPUT_DIR=work_dirs/eval_output/Activitynet_Zero_Shot_QA
CKPT=${1}
...
if [ ! -f "$output_file" ]; then
    for IDX in $(seq 0 $((CHUNKS-1))); do
        ...
        TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=${gpu_devices} python3 hicom/eval/video/inference_video_oqa_activitynet.py \
            --model-path ${CKPT} \
            --video-folder ${EVAL_DATA_DIR}/all_test \
            --question-file ${EVAL_DATA_DIR}/test_q.json \
            --answer-file ${EVAL_DATA_DIR}/test_a.json \
            --output-file ${OUTPUT_DIR}/answers/${CKPT_NAME}/${CHUNKS}_${IDX}.json \
            --num-chunks $CHUNKS \
            --chunk-idx $IDX \
            --dtype bfloat16 &
    done
...
python3 hicom/eval/video/eval_video_oqa_activitynet.py \
    --pred-path ${output_file} \
    --output-dir ${OUTPUT_DIR}/answers/${CKPT_NAME}/gpt \
    --output-json ${OUTPUT_DIR}/answers/${CKPT_NAME}/results.json \
